...
class BCQ():
    ...
    def __load_dataset(self):
        offline_buffer = ts.data.VectorReplayBuffer.load_hdf5('dqn/offline_dataset') 
        unpacked_data = []
        idx = np.arange(len(offline_buffer))
        for key in ['obs', 'act', 'obs_next', 'rew', 'done']:
            data = torch.from_numpy(offline_buffer.get(idx, key=key)).to(self.cfg.device)
            if key not in ['act', 'done']:
                data = data.float()
            unpacked_data.append(data)
        self.buffer = TensorDataset(*unpacked_data)
    ...
    def __eval_generative(self):
        ...
        with torch.no_grad():
            ...
            print('Generative f1-score:', f1_score(actions.cpu(), predicted.cpu()))
